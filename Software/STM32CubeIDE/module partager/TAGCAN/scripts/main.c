...
void run_all_tests(void)
{
    ...
    init_data_container();
    printf("\tpreparation du GT\n");
    TEST_EQ_INT("prepa... ",set_tag("BIT_1", 1), CAN_TG_SUCCESS);
    TEST_EQ_INT("prepa... ",set_tag("BIT_3", 3), CAN_TG_SUCCESS);
    TEST_EQ_INT("prepa... ",set_tag("BIT_4", 4), CAN_TG_SUCCESS);

    // Tag inexistant (doit retourner -3)
    printf("\tTag inexistant\n");
    TEST_EQ_INT("test set",set_value("FANTOME", 5), CAN_TG_ERROR_TAG_NOT_FOUND_);
    TEST_EQ_INT("test get",get_value("FANTOME", &val), CAN_TG_ERROR_TAG_NOT_FOUND_);

    // Dépassement de capacité (doit retourner -4)
    printf("\tDépassement de capacité\n");
    // - Un tag de 1 bit ne peut contenir que 0 ou 1
    TEST_EQ_INT("Un tag de 1 bits[0 ou 1]",set_value("BIT_1", 2), CAN_TG_ERROR_VALUE_TO_BIG);
    // - Un tag de 3 bits ne peut contenir que de 0 à 7
    TEST_EQ_INT("Un tag de 3 bits [0 à 7]",set_value("BIT_3", 8), CAN_TG_ERROR_VALUE_TO_BIG);
    // - Un tag de 4 bits ne peut contenir que de 0 à 15
    TEST_EQ_INT("Un tag de 4 bits [0 à 15]", set_value("BIT_4", 16), CAN_TG_ERROR_VALUE_TO_BIG);
    printf("OK\n");
    ...
    TEST_EQ_INT("val: [256]",set_value("V8", 256), CAN_TG_ERROR_VALUE_TO_BIG); // Erreur dépassement
    printf("OK\n");
