...
void print_all_tag_set(void){
    printf("-------------------------------------------------\n");
    printf("legende\n\tnb:\tn_bits\t (le nombre de bites attribuer � la valeurs)\n");
    printf("\tbia:\tbyte_idx_a\t (le num�ro du bytes de la partie A de la valeur \n");
    printf("\tbib:\tbyte_idx_b\t (le num�ro de bytes de la partie B de la valeur (-1: pas de partie B))\n");
...
void print_all_data(void){
    ...
    printf("format: id\ttag[n.bits]:\tvaleur\n");
    int32_t result_val;
    for(int i = 0; i <= g_tag_count - 1; i++){
        printf("\t%d\t",i);
        printf("%s\t",g_tags[i].name);
...
void print_repartition_bit(void){
    ...
    for(int i = 0; i <= g_tag_count - 1; i++){
        printf("%s :\t",g_tags[i].name);
        printf("[%c]\n", possible_id[i]);
        int bpa = g_tags[i].bit_pos_a;
        int bpb = g_tags[i].bit_pos_b;
        int nb = g_tags[i].n_bits;
        if(nb > 8){
            // 1) pour toute les valeurs pr�sent dans le a, les 8 bites vont �tre attribuer � celui-ci.
            for(int i2=0; i2<8;i2++){

                gbc2[bpa].bi[i2] = possible_id[i];
            }
            gbu[bpa] += 8;
            int i2=gbu[bpb];
            int condi_fin = gbu[bpb]+(nb - 8);
            for(i2; i2<condi_fin;i2++){
                gbc2[bpb].bi[i2] = possible_id[i];
            }
            gbu[bpb] += (nb - 8);
            if(bpb>max_byte_toucher){max_byte_toucher=bpb;}
            if(bpa>max_byte_toucher){max_byte_toucher=bpa;}
        }
        else{
            gbu[bpa] += 8;
            int i2=gbu[bpa];
            int condi_fin = gbu[bpa]+(nb - 8);
            for(i2; i2<condi_fin;i2++){
                gbc2[bpa].bi[i2] = possible_id[i];
            }
            gbu[bpb] += (nb - 8);
    ...
    printf("\ntrame:\n");
    printf("-----------------------------------------------\n\t");
    for(int i = 0; i < max_byte_toucher - 2 ; i++){
                //printf("%s\t",gbc2[i]);
